...
export default defineConfig({
  ...
  retries: 0,
  reporter: 'list',
  outputDir: path.join(os.tmpdir(), 'luster-client-profile-browser-results'),
  use: { baseURL: 'http://127.0.0.1:3148', trace: 'retain-on-failure' },
  webServer: {
    command: 'node node_modules/vite/bin/vite.js --config tests/browser/clientProfile/vite.config.ts',
