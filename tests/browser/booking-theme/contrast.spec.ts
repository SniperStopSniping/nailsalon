...
for (const palette of CUSTOMER_SITE_PALETTE_PRESETS) {
  test(`${palette} key booking text meets 4.5:1 in standard and easier-reading modes`, async ({ page }) => {
    ...
    for (const mode of ['standard', 'easy']) {
      await expect(page.locator('[data-booking-readability]')).toHaveAttribute('data-booking-readability', mode);
      ...
      for (const pair of pairs) {
        expect(contrast(pair.foreground, pair.background), `${palette}/${mode}: ${pair.text}`).toBeGreaterThanOrEqual(4.5);
      }
      await page.getByRole('button', { name: /Easier to read/ }).click();
    }
  });
}
