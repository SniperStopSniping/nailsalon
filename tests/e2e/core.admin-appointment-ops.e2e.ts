...
test.skip(e2eConfig.freeSolo, 'Advanced multi-staff appointment operations are hidden in the free-solo profile.');
...
async function createCustomerBookingWithAddOn(browser: Browser) {
  ...
  try {
    ...
    await page.getByLabel('Customer name').fill(`Ops ${phone.slice(-4)}`);
    await page.getByLabel('Customer email').fill(`ops+${phone}@example.com`);
