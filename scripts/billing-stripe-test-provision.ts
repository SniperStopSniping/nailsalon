 ...
 *     `--allow-host` says. "Production host" means an apex domain in
 *     `PRODUCTION_WEBHOOK_DOMAINS` or anything beneath it, the Vercel
 *     production alias, or this project's `git-main` alias — but NOT the other
 *     `isla-nail-studio-*.vercel.app` hosts, which are the Preview deployment
 *     URLs the rehearsal exists to register against. This tool provisions TEST
 *     MODE for a Preview deployment and has no legitimate reason to point
 *     Stripe at the live salon. The url it will register, and the host it was allowed for, are
...
/**
 * Production apex domains. The host is refused when it EQUALS one of these or
 * is a subdomain of one, so a future `app.lustergel.app` or a repointed
 * `staging.islanailsalon.com` is covered without another edit.
 *
 * `lustergel.app` is the live domain; `islanailsalon.com` currently 404s but is
 * kept because DNS can be repointed and a refusal costs nothing. Production
 * activation is an owner-executed runbook procedure with live-mode keys — it is
 * categorically not something this TEST-MODE provisioner performs, so no flag
 * unlocks these.
 */
export const PRODUCTION_WEBHOOK_DOMAINS: readonly string[] = [
  'lustergel.app',
  'islanailsalon.com',
];

/**
 * The project's Vercel PRODUCTION aliases:
 *   - `isla-nail-studio.vercel.app` — the production deployment;
 *   - `isla-nail-studio-sniperstopsnipings-projects.vercel.app` — Vercel's
 *     `<project>-<scope>.vercel.app` production domain, which is never a
 *     Preview host;
 *   - `isla-nail-studio-git-main-*.vercel.app` (pattern below) — the git alias
 *     for `main`.
 * All three serve production.
 *
 * Every OTHER `isla-nail-studio-*.vercel.app` host carries a deployment hash
 * (`-<hash>-<scope>`) or a non-`main` git branch (`-git-<branch>-<scope>`) and
 * is a Preview deployment URL — precisely what the rehearsal must register
 * against. Matching the project prefix alone would refuse the only hosts this
 * tool exists to serve.
 */
export const PRODUCTION_VERCEL_ALIASES: readonly string[] = [
  'isla-nail-studio.vercel.app',
  'isla-nail-studio-sniperstopsnipings-projects.vercel.app',
];
export const PRODUCTION_VERCEL_GIT_ALIAS_PATTERN = /^isla-nail-studio-git-main-[a-z0-9-]+\.vercel\.app$/;

/** Kept as the flat list of exact apex/www hosts, for messages and tests. */
export const PRODUCTION_WEBHOOK_HOSTS: readonly string[] = [
  ...PRODUCTION_WEBHOOK_DOMAINS,
  ...PRODUCTION_WEBHOOK_DOMAINS.map(domain => `www.${domain}`),
  ...PRODUCTION_VERCEL_ALIASES,
];

/**
 * True for a host this tool refuses outright, whatever `--allow-host` claims:
 * a production apex domain, anything beneath one, a Vercel production alias, or
 * the `git-main` alias of this project.
 */
export function isProductionWebhookHost(host: string): boolean {
  const normalized = normalizeHostname(host);
  if (PRODUCTION_VERCEL_ALIASES.includes(normalized)) {
    return true;
  }
  if (PRODUCTION_VERCEL_GIT_ALIAS_PATTERN.test(normalized)) {
    return true;
  }
  return PRODUCTION_WEBHOOK_DOMAINS.some(
...
export function assertWebhookHostAllowed(url: string, allowHost: string | null): string {
  ...
  if (isProductionWebhookHost(host) || isProductionWebhookHost(allowed)) {
    throw new RefusalError(
      `refusing a production host (${isProductionWebhookHost(host) ? host : allowed}). `
