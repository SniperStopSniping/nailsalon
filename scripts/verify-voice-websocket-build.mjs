...
for (const route of routes) {
  ...
  const external = files.find(file => /\/node_modules\/ws\/(?:index\.js|wrapper\.mjs)$/u.test(file));
...
const websocketModule = await import(pathToFileURL(websocketEntry).href);
const WebSocket = websocketModule.default;
const WebSocketServer = websocketModule.WebSocketServer ?? WebSocket.WebSocketServer;
