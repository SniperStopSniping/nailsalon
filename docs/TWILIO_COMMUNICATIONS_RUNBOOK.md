...
Current operating state (2026-09-09): **Luster SMS is live with explicit user approval.** Protected-main v1.89.3 / `601807ad1b6748b2f3f38f0dae2cf2db101140f4` is deployed as `dpl_BcEWwTeXAyu2ENWPSkTrf8qRDZW2`. The production environment SMS flag and platform switch are on; the disposable-salon pilot restriction is off. Existing per-salon preferences, recipient consent, credit checks and quiet hours remain enforced. No number purchase or paid-plan change was made.

One approved customer booking in the verified `isla-nail-studio` workspace was automatically confirmed. Its SMS was delivered at 13:50:20 UTC, recorded in owner history, charged once, and independently confirmed received by the user. Daniela's missed one-time 100-credit starter allowance was corrected in the existing identity/grant/ledger records after eligibility checks. The test appointment was then cancelled and its unsent reminder invalidated. The cancellation text also delivered once; final balance is 98 available with zero reserved credits. See the current checkpoint in `RESUME.md` and the [pilot checklist](TWILIO_PILOT_CHECKLIST.md) for the limits of this evidence. This current state supersedes the historical paused/release-pending snapshots below.

The current code and deployment configuration govern behavior; older Gate A/B descriptions of a future dispatcher or native-only manual texting are obsolete.
...
Existing salon edits, completed-onboarding replays and previously created businesses are not silently backfilled. Before correcting an older missed allowance, verify the exact salon/owner and prior `billing_starter_grant` evidence, including related durable identities and purged-salon evidence. No admin correction endpoint currently exists. A correction requires an explicitly approved, reviewed, tenant-scoped operation using `resolveOrCreateBusinessIdentity` and `grantStarterCredits` transactionally, with the documented production guards, a verified restorable backup and before/after evidence. Stop on prior grants, conflicting identities or uncertain owner continuity; never edit cached balances directly. A zero balance alone does not prove a missing grant: credits may already have been consumed. Credits count SMS segments, so long or Unicode messages can consume more than one credit. No production starter grant has been made during this setup pass.

Paid credit sales are a separate activation gate. The current `stripePriceMap.ts` top-up mappings are unconfigured in every environment. Production metadata also has no `BILLING_TOPUPS_ENABLED` or dedicated `STRIPE_BILLING_WEBHOOK_SECRET`. Existing Checkout and verified-payment fulfillment cannot sell credits until the approved environment-specific catalog, dedicated webhook and billing switches are configured under the billing contract's sections 12 and 20. The usage API now exposes only locally configured, audience-eligible offers when the top-up switch is enabled, plus explicit purchase availability; no provider call or Stripe ID is exposed. The owner screen shows purchases unavailable otherwise, including when an older API omits readiness. Its click handler also rejects unavailable or stale offers. A legitimate starter grant can fund a pilot without activating paid sales. Do not create fake purchases or edit cached balances to bypass the gate.
...
Operational readiness is a configuration and saved-state check. It does not call Twilio, verify that the sender pool contains a number, test provider permissions, prove Redis connectivity, check callback reachability, or prove the cron has run. Those are separate operational checks; the approved setup follow-up and its provider evidence are recorded in `RESUME.md`. Carrier delivery and authentic callback verification remain part of the separately approved pilot.
