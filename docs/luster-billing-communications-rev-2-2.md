...
Owner-facing and super-admin billing displays report, from a live `billing_subscription` row (status not in `canceled`, `incomplete_expired`) when one exists and from the legacy columns otherwise, what the salon's billing actually is. The owner-facing surface returns that derived answer as `billingMode` directly. The super-admin surface reports it as a separate read-only field, keeping `settings.billingMode` as the stored legacy column, because its editable control is seeded from that field and resubmits it on every save. In neither case is the legacy column rewritten by the new track.
