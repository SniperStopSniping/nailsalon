...
- Shared E2E calendar helper now waits for `aria-pressed="true"` instead of treating the selected date as disabled; it avoids requesting availability again for an already-selected date and opens the optional month when an API-discovered date is outside the week. Confirmation keyboard coverage includes the new reading control before Edit. Enabled-slot and confirmation-navigation assertions remain intact.
...
- `tests/e2e/support/booking.ts` — adapt selection/navigation checks to the accessible week/month calendar without changing slot eligibility assertions.
