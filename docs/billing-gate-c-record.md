...
|---|---|---|---|
| a | **RATIFIED 2026-09-14 (owner) — implemented in P6b.** Real Stripe Price/Coupon IDs are carried by an env-keyed server-only variable (`BILLING_STRIPE_PRICE_IDS` = `{ env, offers, topups, coupons }`, rejected at boot when `env !== BILLING_PLAN_ENV`), with the committed `stripePriceMap.ts` tables staying null placeholders | §4 (mapping "via `BILLING_PLAN_ENV`"), §12 (preview/local cannot resolve prod mappings) | §4 forbids committing live IDs, so activation has no carrier today; the boot check keeps the isolation structural |
| b | Register `/api/billing/windows/evaluate` and `/api/billing/reconcile` as Vercel crons while billing is dark (routes answer `200 skipped`) | §20 (no cron-registration step) | RATIFIED 2026-09-14 (D3) — registered in P4b; costs ~120 cold-start invocations/day against the shared database |
| c | **RATIFIED 2026-09-16 (owner) — the NARROW D19c amendment, implemented in PR-A.** Isolate the whole new-track lifecycle at the legacy `/api/webhooks/stripe` route, not only checkout: Guard A ignores a Checkout Session whose `metadata.purpose ∈ {plan_subscription, sms_topup}` AND a Subscription whose `metadata.purpose = plan_subscription` — the latter inside `syncSubscription`, which is the single funnel for `customer.subscription.*` **and** `invoice.payment_*`; Guard B additionally ignores a Subscription for which a `billing_subscription` row already exists (one indexed read). Objects matching neither guard are processed byte-identically; skipped events answer 200 with no salon row, no audit row and no PII in the log. The CI zero-diff pin on the route is replaced by a reviewed-postimage hash (D18 mechanism). The settings/Portal companion is in scope; no broader contract amendment is approved. | §5 (rewritten: frozen except the isolation exception, plus the normative display companion), §8.1 ("untouched except for the §5 isolation exception"), §24 (retirement still deferred; the exception does not advance it) | The legacy handler is purpose-blind and would rewrite salon billing columns for new-track objects if it still receives them. Checkout alone was not enough: subscription and invoice lifecycle events reach the same route and cannot be filtered away operationally for a host that also carries live legacy subscribers |
