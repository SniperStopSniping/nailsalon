...
Public profile preloading is optional, as in Customer AI: a preload error must not tear down an otherwise attached phone session. The receptionist receives an explicit instruction to delegate salon-information requests and never invent missing facts; canonical consultation, catalogue validation, and booking confirmation remain required. Startup diagnostics separate loading the profile, sending context, and saving state, and classify preload failures using fixed labels only.
