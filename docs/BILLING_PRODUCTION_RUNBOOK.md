...
|---|---|---|---|
| `dark` | `deployed` (or `env-file` supplemented by a saved health response) | deployed `/api/health` of that origin | that deployment is dark, plan-env-consistent, and schema-ready |
| `rehearsal` | `deployed` **only** | `--environment preview`, `--cron-secret-env`, `--bypass-secret-env`, `--env-file` (+`--git-branch`), `--webhook-endpoint-file`, `--portal-config-file`, `--cron-proof-file`, `--integrity-report` | the Preview deployment is configured for the test-mode rehearsal |
| `activate-topups` | `deployed` **only** | same, with `--environment production` and live-mode exports | Production is ready for the FIRST switch (all four switches still unset) |
...
| `BILLING_DEPLOYMENT_MARKER` | optional | optional | `vercel env add BILLING_DEPLOYMENT_MARKER preview <branch>`. Purely an evidence label echoed by `GET /api/billing/readiness` as `deploymentMarker: true`. Set it only AFTER the deploy it is meant to stamp, never before — a marker set ahead of the deploy labels the wrong build |
...
**Verification:** before the flip, `npx tsx scripts/billing-readiness-check.ts --target activate-topups --env-source deployed --environment <preview|production> …` (full command in §4) must exit **0**. After the flip, `--target dark` is EXPECTED to exit 4 — that is the switch working, not a regression — and `curl .../api/health | jq .billing.dark` reads `false`. Run `scripts/billing-integrity-check.ts` (§6) — clean.
...
**Verification:** before the flip, `npx tsx scripts/billing-readiness-check.ts --target activate-subscriptions --env-source deployed --environment <preview|production> …` must exit **0** — this target deliberately tolerates `BILLING_TOPUPS_ENABLED=true` (the documented order) while still requiring `PUBLIC_PRICING_ENABLED` and `BILLING_TAX_COLLECTION_ENABLED` to be unset (D11). After the flip: `billing_credit_window` shows one grant for the test subscription; `/api/billing/reconcile`'s next run reports `drift: []` for that subscription id; integrity check clean.
...
| Reconcile notes | `summary.notes` in `/api/billing/reconcile`'s response — `paid_through_uncomparable`, `paid_through_refunded_invoice`, `refund_evidence_voided`, `refund_evidence_reasserted`, `refund_evidence_uncomparable`, `refund_evidence_unverifiable` | Informational; **never counted as drift and never alerted on**. `paid_through_uncomparable` = the remote invoice's lines were truncated so no comparison was made; `paid_through_refunded_invoice` = the local row is behind because that invoice is refunded (correct, not drift); `refund_evidence_voided` = the safety net retracted evidence Stripe no longer backs (see the row above); `refund_evidence_reasserted` = the OPPOSITE correction — a machine `void` that Stripe now contradicts (the charge is fully refunded) was superseded by a fresh applied row, so entitlement stops again and `billing.subscription_refunded` was raised with `extra.source: 'reconcile'`; a `super_admin` void is never re-asserted this way; `refund_evidence_uncomparable` = a re-assert was declined because the invoice's lines are truncated; `refund_evidence_unverifiable` = the invoice could not be retrieved (or bills no subscription line) this pass and will be re-checked next hour. Only a note that repeats every hour for the same subscription warrants investigation |
