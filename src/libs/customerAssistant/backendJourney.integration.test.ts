...
(target ? describe : describe.skip)('customer assistant browser journey through real handlers — PostgreSQL', () => {
  beforeAll(async () => {
    ...
    await database.insert(schema.technicianServicesSchema).values({ technicianId: TECHNICIAN, serviceId: SERVICE, enabled: true }).onConflictDoNothing();
  ...
  it.each(['chromium', 'webkit'] as const)('%s creates one no-deposit appointment after proposal, slot, contact, review, and confirmation', async (engine) => {
    browser = await (engine === 'chromium' ? chromium : webkit).launch();
    const page = await browser.newPage({ viewport: { width: 390, height: 844 } });
    const unexpected: string[] = [];
    const serverResults: Array<{ path: string; status: number; kind?: string; reason?: string; bookingState?: string }> = [];
    await page.route('**/*', async (route) => {
      ...
      serverResults.push({ path: url.pathname, status: response.status, kind: data.result?.kind ?? data.kind, reason: data.result?.reason, bookingState: data.status });
    ...
    await page.screenshot({ path: path.resolve(process.cwd(), `artifacts/customer-assistant/actual-backend-${engine}-confirmed.png`), fullPage: true });
