...
export async function evaluateReceptionistTurn(intent: z.infer<typeof customerInterpretationSchema>, conversation: CustomerConversation, turn: ReceptionistEvalTurn) {
  ...
  const result = await resolveCustomerTurn({ salonId: 'synthetic-isla', salonSlug: 'synthetic-isla', features: null, locale: 'en' }, SEMANTIC_L1_MENU, intent, conversation, next, {
    ...
    lookupCustomerSlots: async args => ({ proposal: await buildProposal(args.salon.id, args.features, args.selection), today: '2026-09-18', timeZone: 'America/Toronto', slots: [{ time: args.preference.earliest < '09:00' ? '09:00' : args.preference.earliest, startTime: new Date(Date.parse(`${args.preference.date}T${args.preference.earliest < '09:00' ? '09:00' : args.preference.earliest}:00-04:00`)).toISOString() }], selected: null, quoteChanged: false }),
    lookupNextCustomerSlots: async args => ({ proposal: await buildProposal(args.salon.id, args.features, args.selection), today: '2026-09-18', timeZone: 'America/Toronto', preference: { date: '2026-09-19', earliest: '00:00', latest: '23:59' }, slots: [{ time: '12:30', startTime: '2026-09-19T16:30:00.000Z' }], selected: null, quoteChanged: false }),
  });
