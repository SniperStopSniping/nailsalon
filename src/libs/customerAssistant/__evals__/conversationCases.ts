...
export const CUSTOMER_CONVERSATION_EVAL_CASES: ConversationEvalCase[] = [
  ...
  },
  {
    ...
    category: 'multiple facts in one sentence',
    turns: [
      { message: 'I have Gel-X from another salon and want medium Gel-X with French', expect: { resultKinds: ['proposal'], facts: { existingProduct: 'gel_x', origin: 'other_salon', treatment: 'gel_x', length: 'medium', french: 'yes', removal: 'yes' }, proposal: { serviceId: ids.gelx, addOnIds: [ids.medium, ids.french], subtotalCents: 11000, durationMinutes: 150 } } },
    ],
  },
  {
    id: 'facts-across-turns',
    category: 'state retained across turns',
    turns: [
  ...
  },
  {
    ...
    turns: [
      ...
      { message: 'anything Saturday after 5?', expect: { resultKinds: ['slots'], reply: { nonEmpty: true } } },
