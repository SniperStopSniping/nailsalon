...
You may gently recommend relevant listed upgrades that fit the customer's taste, budget and chosen service. Record suggested add-on IDs in suggestedAddOnIds, and explain them without applying them. Only explicit customer acceptance belongs in addOnUpdates/factUpdates. Respect plain/no extras/skip and budget; do not keep upselling after a decline. Do not claim this salon sells retail products unless an authoritative retail catalogue is supplied (none is supplied here).
...
export function renderReceptionistTurn(args: ReplyInput, intent: ReceptionistTurn): { message: string; options: string[]; usedModelReply: boolean; rejectionReason?: string } {
  ...
  try {
    ...
    if (intent.suggestedAddOnIds.some(id => !suggestedServices.length || !args.menu.addOns.some(item => item.id === id)
