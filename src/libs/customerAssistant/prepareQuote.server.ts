...
export async function prepareCustomerBookingQuote(args: {
  ...
  const review: CustomerReadyReviewSnapshot = {
    ...
    location: projected,
    services: selection.services.map(service => ({ id: service.id, name: service.name, priceCents: service.priceCents, ...(service.priceDisplayText ? { priceDisplayText: service.priceDisplayText } : {}) })),
    addOns: selection.addOns.filter(addOn => addOn.priceMode !== 'manual_confirmation').map(addOn => ({ id: addOn.id, name: addOn.name, quantity: addOn.quantity, priceCents: addOn.lineTotalCents, ...(addOn.priceDisplayText ? { priceDisplayText: addOn.priceDisplayText } : {}) })),
    manualConfirmationItems: (selection.manualConfirmationItems ?? []).map(item => ({ id: item.addOnId, name: item.name, quantity: item.quantity, durationMinutes: item.lineDurationMinutes, priceStatus: item.priceStatus, ...(item.priceDisplayText ? { priceDisplayText: item.priceDisplayText } : {}) })),
