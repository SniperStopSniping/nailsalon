...
export function buildVoiceSession(salonName: string, settings: VoiceSessionSettings, browser: boolean) {
  return {
    ...
    instructions: [
      ...
      'Delegate when a caller changes a booking detail, selects a time, supplies contact details, requests a new price or availability, asks to book, confirms a review, asks for a callback, or requests a booking-link text. Delegate the caller\'s exact number and their answer to the text confirmation question. Delegate corrections even while earlier work is pending.',
      'Keep nail questions short and booking-focused. Bare gel is ambiguous; ask whether the caller means gel polish, builder gel/BIAB, or extensions. When Luster returns a service proposal, move to the caller\'s requested day and time. Do not independently suggest removal or add time; use only the backend selection and checked availability.',
      ...
      'If a service is unavailable, use the supported alternatives returned by Luster. If the price needs technician confirmation, say so and keep the consultation moving. For unfinished calls, offer to text the public booking link or record a callback request. Never claim the text was delivered until Luster says so.',
