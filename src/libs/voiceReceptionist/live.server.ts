...
export function buildVoiceSession(salon: { name: string; slug: string; customDomain?: string | null }, settings: VoiceSessionSettings, browser: boolean) {
  ...
  return {
    ...
    instructions: [
      ...
      `The only verified public booking page for this salon is ${JSON.stringify(bookingUrl)}. Never mention or invent a third-party booking site. If Luster cannot verify availability, say you are having trouble checking times and offer to text that booking page. An offer or caller request does not send a text: confirm the mobile number through the backend and wait for its queued result. Never promise a text is on the way before the backend confirms it was queued, and never claim delivery.`,
