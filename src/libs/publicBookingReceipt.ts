...
export function publicBookingReceiptPresentation(response: any, timeZone: string) {
  ...
  return {
    ...
    addOns: data.addOns.map((item: any) => ({ id: (item.addOnId ?? item.id) as string, name: item.nameSnapshot as string, price: item.unitPriceCentsSnapshot / 100, duration: item.durationMinutesSnapshot as number, quantity: item.quantitySnapshot as number })),
