 ...
 *    file.
 *
 * Every caller is responsible for supplying what it wants evaluated.
 *
 * This is a DIAGNOSTIC tool only: the actual boot-time isolation
...
function checkProvisionedWebhookEndpoint(
  ...
  try {
    ...
    // A query string on the registered endpoint URL is how a Vercel
    // deployment-protection bypass token ends up stored in — and displayed by
    // — the shared Stripe dashboard (O6). Refuse it here rather than quietly
    // comparing only origin+path and letting the token ride along.
    if (endpointUrl.search !== '') {
      return { id: 'provisioned_webhook_endpoint', ok: false, detail: 'endpoint URL carries a query string; a bypass token must not be stored in the Stripe endpoint URL (O6)' };
    }
...
export type BillingPortalConfigurationExport = {
  ...
  is_default: boolean;
  /**
   * REQUIRED. A portal export with no mode marker cannot be attributed to the
   * test or live account at all, and "assume it matches" is exactly the
   * assumption D16 exists to remove.
   */
  livemode: boolean;
...
/**
 * BLOCKER fix: `--environment` is an OPERATOR ASSERTION about which deployment
 * is being proven — it selects the Preview-only 503 allowance and, through the
 * target, the expected plan env and key mode. Left uncoupled, an operator could
 * label a production deployment `preview`, unlock the degraded-health
 * allowance, and reach exit 0 on an activation target. The deployment's own
 * `vercelEnv` is the only thing that can contradict that assertion, so it must.
 */
function evaluateDeclaredEnvironment(
  collector: CheckCollector,
  evidence: BillingReadinessEvidence,
  readiness: BillingReadinessEndpointFacts,
): void {
  collector.add(
    ...
    evidence.environment !== null && evidence.environment === readiness.vercelEnv,
    `--environment declared "${evidence.environment ?? '(unspecified)'}"; the deployment reports vercelEnv="${readiness.vercelEnv ?? 'null'}"`,
  );
}

/**
 * MINOR 7: the two deployed evidence reads are separate HTTP requests and can
 * land on different deployments (a redeploy mid-run, or two different origins).
 * Their `gitSha` values are the only shared identity, so they must agree.
 */
function evaluateDeploymentIdentity(
  collector: CheckCollector,
  evidence: BillingReadinessEvidence,
): void {
  // Only meaningful when BOTH payloads are present; a payload that was never
  // supplied is reported by its own missing-evidence entry, not here.
  if (evidence.health === undefined || evidence.readiness === undefined) {
    return;
  }
  const healthSha = evidence.health.gitSha;
  const readinessSha = evidence.readiness.gitSha;
  if (healthSha === null && readinessSha === null) {
    // Neither surface names a commit (an unhosted runtime sets no
    // VERCEL_GIT_COMMIT_SHA at all): there is nothing to compare, and
    // inventing a failure would not make the two reads any more correlated.
    return;
  }
  if (healthSha === null || readinessSha === null) {
    // ONE side names a commit and the other does not. On a real deployment
    // both read the same environment variable, so this is two different
    // runtimes answering — or a hand-edited evidence file. Never a pass.
    collector.add(
      'deployment_sha_consistent',
      false,
      `only one deployed surface names a commit (/api/health "${healthSha ?? 'null'}", /api/billing/readiness "${readinessSha ?? 'null'}"); on one deployment both read the same VERCEL_GIT_COMMIT_SHA`,
    );
    return;
  }
  collector.add(
    'deployment_sha_consistent',
    healthSha === readinessSha,
    `/api/health reports gitSha "${healthSha}"; /api/billing/readiness reports "${readinessSha}"`,
  );
}
...
function evaluateDeployedHealth(
  ...
  // The aggregate `status` is carried in this check's DETAIL rather than being
  // its own check. A Preview deployment is structurally `degraded` (redis /
  // Resend / Google Calendar are not provisioned on the branch scope, handoff
  // §6 row X7), so an aggregate-status check could only ever be informational
  // — and a check whose `ok` is a constant reads like a gate without being
  // one. `schemaDrift` is the real gate here; the status travels with it so a
  // reader still sees that the deployment answered 503 and why that is
  // expected.
  collector.add(
    'health_schema_drift_ready',
    health.schemaDrift === 'ready',
    `deployed /api/health schemaDrift="${health.schemaDrift}" (aggregate status="${health.status}", HTTP ${health.httpStatus}${health.status === 'ok' ? '' : ' — a Preview deployment is structurally degraded; reported, not a billing-verdict term'})`,
  );
}
...
type CronBodyExpectation = 'dark-skip' | 'no-skip' | 'any-record';

/**
 * What each cron's 200 body must look like for a given target.
 *
 * The two jobs gate DIFFERENTLY, and conflating them produces a false
 * negative that would block a correct activation:
 *  - `/api/billing/windows/evaluate` returns `{skipped:'BILLING_DISABLED'}`
 *    whenever `BILLING_SUBSCRIPTIONS_ENABLED` is not `'true'`. Both activation
 *    targets require that switch to still be unset, so the dark skip is the
 *    only correct answer for both.
 *  - `/api/billing/reconcile` skips only when NEITHER switch is set; with
 *    `BILLING_TOPUPS_ENABLED='true'` it still reconciles held top-ups and
 *    answers `{purged, topups}` with no `skipped` key at all. Under D11 that
 *    is exactly the state `activate-subscriptions` is proven in, so demanding
 *    a skip there would refuse the documented activation order. When top-ups
...
function expectedCronBody(
  ...
  // No readiness payload ⇒ the switch state is unknown; require the stricter
  // dark skip rather than accepting whatever was recorded. (That case already
  // fails on its own missing-evidence entry.)
  return switches === null || switches.topups === false ? 'dark-skip' : 'no-skip';
}
...
function evaluateCrons(
  ...
  for (const path of BILLING_CRON_PATHS) {
    ...
    if (body === null) {
      failures.push(`${path} recorded no response body object`);
      continue;
    }
    switch (expectedCronBody(target, path, evidence.readiness?.switches ?? null)) {
      case 'dark-skip':
        if (body.skipped !== 'BILLING_DISABLED') {
          failures.push(`${path} did not answer {"skipped":"BILLING_DISABLED"}; that job is not dark on this deployment`);
        }
        break;
      case 'no-skip':
        if ('skipped' in body) {
          failures.push(`${path} answered a "skipped" body, but with BILLING_TOPUPS_ENABLED already true it reconciles held top-ups and cannot skip`);
        }
        break;
      case 'any-record':
      default:
...
function evaluatePortalConfiguration(
  ...
  // No `undefined` tolerance: an export with no mode marker is a portal
  // configuration from an unknown account, not a passing one.
  const livemodeOk = configuration.livemode === expectsLive;
  collector.add(
    'portal_subscription_update_disabled',
...
export function evaluateBillingReadiness(input: {
  ...
  if (evidence.readiness !== undefined) {
    evaluateDeclaredEnvironment(collector, evidence, evidence.readiness);
  }
  evaluateDeploymentIdentity(collector, evidence);
  ...
  if (target === 'dark') {
    ...
    evaluateCrons(collector, target, evidence);
