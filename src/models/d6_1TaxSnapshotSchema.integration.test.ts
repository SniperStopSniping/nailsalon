...
describe('migration 0068 — D6.1 invoice and tax snapshot foundation', () => {
  ...
  it('pins the immutable Stripe prerequisite before the Review migration', () => {
    ...
    expect(journal.entries).toHaveLength(83);
    ...
    expect(journal.entries[82]).toMatchObject({ idx: 82, tag: '0083_review_request_manual_contract' });
  ...
  it.each([
    ...
    [77, 'Stripe 0076'],
    [78, 'Review 0077'],
  ])('upgrades a %s ledger through review request manual contract 0083', async (existingCount) => {
