...
export default function StaffClientProfilePage() {
  ...
  // Staff auth via the staff session (phone OTP), NOT Clerk — techs don't have Clerk accounts
      ...
      try {
        ...
        if (!cancelled) {
          if (response.ok) {
            ...
            setIsAuthenticated(Boolean(data.data?.technician));
          } else {
  ...
  useEffect(() => {
    if (authChecked && isAuthenticated && salonSlug && phone) {
      fetchProfile();
    }
  }, [authChecked, isAuthenticated, salonSlug, phone, fetchProfile]);
  ...
  if (!isAuthenticated) {
    return (
      ...
      >
        ...
        <p className="text-neutral-600">Your staff session has expired. Please sign in again.</p>
        <button
          type="button"
          onClick={() => router.push('/staff-login')}
          className="mt-6 rounded-full px-6 py-3 text-sm font-bold text-white transition-opacity hover:opacity-90"
                ...
                >
                  ...
                  >
                    <div className="p-4">
                      ...
                      {/* Stats (hidden entirely when the salon redacts client history) */}
                      {profile.stats.totalVisits !== undefined && (
