...
describe('POST /api/salon/add-ons', () => {
  ...
  it('allows manual confirmation only on a selected compatible service', async () => {
    const response = await POST(postRequest({ ...VALID_BODY, serviceIds: [SERVICE_A], manualConfirmationServiceIds: [SERVICE_A] }));
    const body = await response.json();
