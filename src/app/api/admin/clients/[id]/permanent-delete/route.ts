...
const permanentDeleteRequestSchema = z.object({
    ...
    case 'UNSUPPORTED_CLIENT_IDENTITY':
      return privateJson({
        error: {
          ...
          message: 'This client can’t be permanently deleted safely.',
        },
      }, { status: 409 });
...
export async function POST(
  ...
  try {
    ...
    const parsed = permanentDeleteRequestSchema.safeParse(await request.json());
    if (!parsed.success) {
      return privateJson({
        error: {
    ...
    console.error('Error permanently deleting client');
    return privateJson({
      error: {
        code: 'INTERNAL_ERROR',
