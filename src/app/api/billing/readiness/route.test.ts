...
// Scanner-safe fixture keys: the repo's secret scanner flags
// `sk_(test|live)_` followed by 16+ alphanumerics, so the suffix here is
// deliberately short. The route only reads the PREFIX (`resolveStripeKeyMode`),
// so these still exercise the `test` / `live` / `null` branches exactly.
const FIXTURE_TEST_KEY = 'sk_test_placeholder';
const FIXTURE_LIVE_KEY = 'sk_live_placeholder';
...
function populateRehearsalEnvironment(): void {
  ...
  process.env.STRIPE_SECRET_KEY = FIXTURE_TEST_KEY;
...
describe('GET /api/billing/readiness — authorization', () => {
  ...
  it('is never cached — including the refusal, which must not be replayed to an authorized caller', async () => {
...
describe('GET /api/billing/readiness — no value-shaped strings', () => {
  it('never serializes a secret, key, price, coupon or promotion id', async () => {
    ...
    expect(body).not.toContain(FIXTURE_TEST_KEY);
    expect(body).not.toContain('placeholder');
    expect(body).not.toContain(CRON_SECRET);
  });

  it('never echoes a deployment-protection bypass token carried in NEXT_PUBLIC_APP_URL', async () => {
    populateRehearsalEnvironment();
    process.env.NEXT_PUBLIC_APP_URL = 'https://preview-host.example/?x-vercel-protection-bypass=bypass-token-fixture';

    const response = await call();
    const body = await response.text();

    // `appOrigin` is normalised through `new URL(...).origin`, which discards
    // the query entirely — the token can never ride out on this surface.
    expect(JSON.parse(body).appOrigin).toBe('https://preview-host.example');
    expect(body).not.toContain('bypass-token-fixture');
    expect(body).not.toContain('x-vercel-protection-bypass');
  });

  it('clamps an out-of-range BILLING_PLAN_ENV to null instead of echoing the value', async () => {
    populateRehearsalEnvironment();
    process.env.BILLING_PLAN_ENV = 'sentinel-not-a-plan-env';

    const body = await (await call()).text();

    expect(JSON.parse(body)).toMatchObject({ planEnv: null, planEnvMatchesRuntime: false });
    expect(body).not.toContain('sentinel-not-a-plan-env');
  });
});
...
describe('GET /api/billing/readiness — presence facts', () => {
  ...
  it('reports a live-mode key and a plan-env mismatch without throwing', async () => {
    populateRehearsalEnvironment();
    process.env.STRIPE_SECRET_KEY = FIXTURE_LIVE_KEY;
