  ...
  if (view === 'removed-sections') {
    return (
      <div className="final-structure final-structure--subview">
        ...
        <div className="final-structure__section-heading">
          <div>
            <span>Recoverable content</span>
            <strong>Removed sections</strong>
          </div>
          <b>{document.unusedSections.length}</b>
        </div>
              ...
              <ul aria-label="Removed sections" className="final-structure__recovery-list">
                {document.unusedSections.map(section => (
                  <li data-section-id={section.id} key={section.id}>
                    <span>
                      <strong>{getSectionOwnerIdentity(section).label}</strong>
                      <small>{getSectionOwnerIdentity(section).recoveryDetail}</small>
                    </span>
                    <button aria-label={`Restore ${section.label} to the current page`} type="button" onClick={() => onRestoreSection(section)}>
    ...
    return (
      <div className="final-structure final-structure--subview">
        ...
        <div className="final-structure__section-heading">
          ...
          <b>{document.removedPages.length}</b>
        </div>
              ...
              <ul aria-label="Removed pages" className="final-structure__recovery-list">
                {document.removedPages.map(record => (
                  <li data-page-id={record.page.id} key={record.page.id}>
                    <span>
                      <strong>{record.page.name}</strong>
                      <small>
                        {record.sectionIds.length}
                        {' '}
                        section
                        {record.sectionIds.length === 1 ? '' : 's'}
                        {' '}
                        retained
                      </small>
                    </span>
                    <button aria-label={`Restore ${record.page.name} page`} type="button" onClick={() => onRestorePage(record.page.id)}>
                      <RotateCcw aria-hidden="true" size={17} />
                      {' '}
                      Restore page
                    </button>
  ...
  return (
    <div className="final-structure" data-final-panel="pages-structure" data-testid="structure-tree">
              ...
              <div className="final-structure__page-row">
                ...
                >
                  ...
                  <span>
                    ...
                    <small>{page.visible ? `${sections.length} section${sections.length === 1 ? '' : 's'}` : 'Page hidden'}</small>
                  </span>
                ...
                ? (
                    <div className="final-structure__active-page">
                      ...
                      <div className="final-structure__page-actions">
                        {sections.length > 1
                          ? (
                              <button type="button" onClick={onEnterReorder}>
      ...
      <div className="final-structure__secondary">
        <button type="button" onClick={() => setView('navigation')}>
          <Menu aria-hidden="true" size={18} />
          <span>
            <strong>Menu</strong>
            <small>{document.navigation.enabled ? 'On' : 'Off'}</small>
          </span>
          <Pencil aria-hidden="true" size={16} />
        </button>
        {document.unusedSections.length > 0
          ? (
              <button type="button" onClick={() => setView('removed-sections')}>
                <RotateCcw aria-hidden="true" size={18} />
                <span>
                  <strong>Removed sections</strong>
                  <small>
                    {document.unusedSections.length}
                    {' '}
                    ready to restore
                  </small>
                </span>
        ...
        {document.removedPages.length > 0
          ? (
              <button type="button" onClick={() => setView('removed-pages')}>
                <RotateCcw aria-hidden="true" size={18} />
                <span>
                  <strong>Removed pages</strong>
                  <small>
                    {document.removedPages.length}
                    {' '}
                    ready to restore
                  </small>
                </span>
