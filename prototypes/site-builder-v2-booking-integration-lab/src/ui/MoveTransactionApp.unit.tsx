...
async function chooseMultiPage(user: ReturnType<typeof userEvent.setup>): Promise<void> {
  ...
  if (!servicesButton) {
    throw new Error('Services & Booking page button is missing.');
  }
...
async function openBookingMove(
  ...
  if (!select) {
    throw new Error('Booking selection surface is missing.');
  }
  if (select.getAttribute('aria-pressed') !== 'true') {
...
describe('transactional shared Move surface', () => {
  ...
  it.each([
    ...
    async (dismissal, resolution) => {
      ...
      if (!bookingId) {
        throw new Error('Booking section id is missing.');
      }
