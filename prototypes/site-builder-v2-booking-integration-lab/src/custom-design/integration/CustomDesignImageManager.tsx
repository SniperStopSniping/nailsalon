    ...
    >
      <div className="custom-design-owner-image-row__preview">
        {previewUrl
          ...
          : (
              <span aria-label="Thumbnail unavailable" role="img">
                {`Page ${index + 1}`}
              </span>
            )}
      </div>
        ...
        {reviewCount > 0
          ? (
              <span className="custom-design-owner-review-label">
                ...
                {' '}
                link
                {reviewCount === 1 ? 'position' : 'positions'}
              </span>
            )
          : null}
      </div>
        ...
        {onLinkAreas
          ? (
              <button type="button" onClick={onLinkAreas}>
                ...
                {' '}
                {image.interactiveAreas.length > 0 ? 'Edit clickable areas' : 'Make something clickable'}
              </button>
            )
          : null}
...
export function CustomDesignImageManager({
  ...
  useEffect(() => {
    if (imageOrderDraft === undefined) {
      setDraftOrder(images.map(image => image.id));
    }
  ...
  const moveImage = (id: string, direction: -1 | 1) => {
    ...
    if (from < 0 || to < 0 || to >= activeDraftOrder.length) {
      return;
    }
