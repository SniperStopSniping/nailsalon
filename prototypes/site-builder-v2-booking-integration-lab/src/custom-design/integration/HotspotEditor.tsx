...
const reorderAreas = (
  ...
  if (from < 0 || to < 0 || to >= ordered.length) return [...areas];
...
export function HotspotEditor({
  ...
  const startPointerSession = (
    ...
    const end = (pointerEvent: PointerEvent) => {
      if (pointerEvent.pointerId === session.pointerId) cleanup();
    };
  ...
  const addArea = () => {
    if (areas.length >= CUSTOM_DESIGN_MAX_AREAS_PER_IMAGE) return;
  ...
  const changeSelectedAction = (action: CustomDesignAction | null) => {
    ...
    setInvalidActionIds((current) => {
      ...
      if (action) next.delete(selectedArea.id);
      else next.add(selectedArea.id);
      return next;
    });
    if (action) updateArea(selectedArea.id, area => ({ ...area, action }));
  };

  const commit = () => {
    if (!image || issues.length > 0) return;
    ...
    >
      <div className="custom-design-owner-hotspot-session">
        <div className="custom-design-owner-hotspot-toolbar">
          <p>{areas.length} of {CUSTOM_DESIGN_MAX_AREAS_PER_IMAGE} areas</p>
          <button
            ...
            onClick={addArea}
          >
            <Plus aria-hidden="true" size={18} /> Add link area
          </button>
        </div>
        ...
        {interactionWarning ? (
          <p className="custom-design-owner-inline-warning" role="status">
            <AlertTriangle aria-hidden="true" size={18} /> {interactionWarning}
          </p>
        ) : null}
          ...
          <section className="custom-design-owner-hotspot-details">
            <div className="custom-design-owner-section-heading">
              <div>
                ...
                <p>Area {selectedArea.semanticOrder + 1} of {areas.length}</p>
              </div>
              <button
                type="button"
                onClick={() => {
              ...
              >
                <Trash2 aria-hidden="true" size={17} /> Remove area
              </button>
            </div>
            <label>
              Accessible label
            ...
            <div className="custom-design-owner-area-order" role="group" aria-label="Link area reading order">
              <span>Reading order: {selectedArea.semanticOrder + 1}</span>
              <button
              ...
              >
                <ArrowUp aria-hidden="true" size={17} /> Earlier
              </button>
              <button
                ...
                onClick={() => setAreas(current => reorderAreas(current, selectedArea.id, 1))}
              >
                <ArrowDown aria-hidden="true" size={17} /> Later
              </button>
            </div>
