...
export function BrandBasicsScreen({
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!profile.businessName.trim()) {
      nextErrors.businessName = 'Add your salon or studio name.';
    }
    if (!profile.businessType) {
      nextErrors.businessType = 'Choose what best describes your business.';
    }
    if (personalBusiness && !profile.ownerName.trim()) {
      nextErrors.ownerName = 'Add your name.';
    }
      ...
      <form id={formId} noValidate ref={formRef} onSubmit={submit}>
        ...
        <section className="onboarding-business-card is-expanded">
          <header>
            ...
            <div>
              <h2>Business name</h2>
              <p>The name of your salon or nail business.</p>
            </div>
          </header>
                      ...
                      ? (
                          <button
                            ...
                            onClick={() => {
                              ...
                              onProfileChange({ siteSlug: normalizeSiteSlug(profile.businessName), siteSlugCustomized: false });
                            }}
                          >
                            Use suggested URL
                          </button>
                        ...
                        <label>
                          ...
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              onProfileChange({ siteSlug, siteSlugCustomized: true });
                            }}
                          />
                        </label>
                      )
        ...
        <fieldset className={`onboarding-business-card onboarding-business-type${errors.businessType ? ' has-error' : ''}`} aria-invalid={errors.businessType ? 'true' : undefined}>
          ...
          <div>
            {BUSINESS_TYPE_OPTIONS.map((option) => {
              ...
              return (
                <label key={option.id}>
                  <input checked={profile.businessType === option.id} name="business-type" type="radio" value={option.id} onChange={() => selectBusinessType(option.id)} />
                  <span>
                    <Icon aria-hidden="true" size={25} />
                    <strong>{option.label}</strong>
                    <small>{option.description}</small>
                    {profile.businessType === option.id ? <Check aria-hidden="true" className="onboarding-business-type__check" size={15} /> : null}
                  </span>
                </label>
              );
            })}
          ...
          ? (
              <section className="onboarding-business-card onboarding-business-card--split">
                <header>
                  ...
                  <div>
                    <h2>Your name</h2>
                    <p>Your name helps clients know who they’re booking with.</p>
                  </div>
                </header>
                <TextField
                  autoComplete="name"
...
export function LocationContactScreen({
  ...
  const submit = (event: FormEvent) => {
    ...
    if (failedFields.length > 0) {
      ...
      if (firstFailedField) {
        revealError(firstFailedField, true);
      }
      return;
    }
    if (
        ...
        <form id={formId} noValidate ref={formRef} onSubmit={submit}>
          ...
          >
            ...
            <details className="onboarding-arrival-details">
              <summary>
                <span>
                  <strong>Arrival details · Optional</strong>
                  <small>Help clients find and enter your location.</small>
                </span>
              </summary>
              <div>
                <TextField
            ...
            <div className="onboarding-shared-instagram">
              <div>
                ...
                <strong>
                  {instagramResolution.status === 'resolved'
                    ? `@${instagramResolution.username}`
                    : profile.instagram.trim() || 'Not added yet'}
                </strong>
