...
const CURATED: Record<LibrarySectionType, Curated> = {
  about: {
    ...
    limitations: 'Bio, specialties, and credentials stay owned by the Business Profile; the editor links there rather than duplicating them.',
  ...
  announcement_bar: {
    ...
    motion: 'Rises in on load (transform+opacity), press-scales its action, dismiss button scales on press; all suppressed under reduced motion.',
  ...
  featured_services: {
    ...
    motion: 'Card lift on hover, image saturation warm-up, arrow nudge on the text CTA.',
  ...
  quick_info: {
    ...
    motion: 'Facts stagger in (60ms apart) on load; suppressed under reduced motion.',
  ...
  visit_us: {
    accessibility: 'Labelled region; the directions link carries the resolved destination as its accessible name.',
